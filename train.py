...
def grid_search(dataset, res, censored, cen_per, fit_type, transform):
    ...
    cols = ["prefix", "Layer_Size", "MAE", "unique_pred", "train_size", "val_size", "test_size","Censored %", "fit_time"]
    ...
    for prefix in range_dict[dataset]:
        ...
        for layer_size in [4,8,16]:
            ...
            grid_results = grid_results.append(
                pd.DataFrame([[prefix,layer_size,mae,nunique,len_train,len_val,len_test,cen_percentage,t2e_obj.fit_time]],columns=cols), 
                ignore_index=True)
    return grid_results
...
#      ## Regression
#     for cen_prc in [0]:
#         mae_path = 'output_files/maes/regression/'+str(cen_prc) +'/'
...
#                             transform=False
#                        )
#         pickle.dump(grid_results, open(mae_path + 't2e_'+dataset+'_GRU.pkl', 'wb'))

# #    T2E Un-censored transform
#     for cen_prc in [0]:
